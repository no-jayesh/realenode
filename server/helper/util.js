...
module.exports = {
  ...
  async adhaarSendOTPRequest(aadhaarNumber) {
    ...
    var configApi = {
      ...
      headers: {
        ...
        'secretKey': config.get('emptra.secretKey'),
        'clientId': config.get('emptra.clientId'),
        'Cookie': config.get('emptra.Cookie')
      },
      data: data
    };
  ...
  async adhaarVerifyOTPRequest(client_id, otp) {
    var data = JSON.stringify({
    ...
    var configApi = {
      method: 'post',
      url: 'https://api.emptra.com/aadhaarVerification/submitOtp',
      headers: {
        'Content-Type': 'application/json',
        'secretKey': config.get('emptra.secretKey'),
        'clientId': config.get('emptra.clientId'),
 ...
 // sent email thanks to enquiry 
      ...
      <div style="margin: 50px auto; width: 70%; padding: 20px 0">
        <div style="border-bottom: 1px solid #eee">
          <table style="width: 100%">
            <tr>
              <th><img src="https://tecmetaverse.com/wp-content/uploads/2024/02/Tech_logo.jpg" alt="Company Logo" style="max-width: 100px;"></th>
        ...
        <p style="padding: 10px; background: #f0f0f0; border-radius: 4px;">
          Thank you for your inquiry about our knights fine states services. We have received your request and one of our agents will be in touch with you soon.
        </p>
        <p style="padding: 10px; background: #f0f0f0; border-radius: 4px;">
          If you have any immediate questions, feel free to contact us at www.knightsfinestates.com.
        </p>
        <p style="padding: 10px; background: #f0f0f0; border-radius: 4px;">
          Best regards,<br/>
        ...
        knightsfinestates | Dubai | 1478523698
        </div>
      </div>
    </div>`;
  ...
  async sendUserListToAdmin   () {
    try {
      ...
      let mailOptions = {
        from: config.nodemailer.email,
        to: config.nodemailer.adminEmail, // Admin email from config
        subject: 'User List',
  ...
  async  getMonthlyUserCount() {
    ...
    for (const month of months) {
      const firstDayOfMonth = new Date(`${month} 1, ${currentYear}`);
      const lastDayOfMonth = new Date(new Date(firstDayOfMonth).setMonth(firstDayOfMonth.getMonth() + 1) - 1);
  
      const individualCount = await userModel.countDocuments({
        userType: 'Individual',
        createdAt: {
          $gte: firstDayOfMonth,
          $lte: lastDayOfMonth
      ...
      const organizationCount = await userModel.countDocuments({
      ...
      userCounts.Individual.push({ month, count: individualCount });
      userCounts.Organization.push({ month, count: organizationCount });
    }
...
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:v="urn:schemas-microsoft-com:vml"
  ...
  sendMessageToAdmin: async (adminEmail, message) => {
    console.log("adminEmail", adminEmail) || console.log("message", message)
    var sub = `<div style="font-family: Helvetica,Arial,sans-serif;min-width:1000px;overflow:auto;line-height:2">
    <div style="margin:50px auto;width:70%;padding:20px 0">
      <div style="border-bottom:1px solid #eee">
        <table style="width:100%">
      ...
      <p style="padding: 10px; background: #f0f0f0; border-radius: 4px;">${message}</p>
      <p style="font-size:0.9em;">Thank You,</p>
      <hr style="border:none;border-top:1px solid #eee" />
      <div style="float:right;padding:8px 0;color:#aaa;font-size:0.8em;line-height:1;font-weight:300">
      </div>
